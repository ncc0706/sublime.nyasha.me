'use strict';var appGallery=function(){function events(){$('.portfolio-ajax').addClass('loading');$(window).on('load',layoutItems);$(window).on('resize',layoutItems);$(document).on('click touchstart','.toggle-sidebar',function(){layoutItems();});$('.superbox').imagesLoaded().always(function(){$('.superbox').fadeIn();$('.portfolio-ajax').removeClass('loading');}).done(function(){$('.gallery-loader').addClass('hide');$('.superbox').removeClass('hide');layoutItems();});}
...
return{init:function(){events();superbox();}};}();$(function(){appGallery.init();});